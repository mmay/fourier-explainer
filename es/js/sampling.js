(function () {
  ...
  function draw() {
    ...
    if (state.sigFreq > nyquist) {
      ...
      statusBox.innerHTML = `<strong style="color:#ff7777">Aliasing.</strong> La señal es de ${state.sigFreq.toFixed(
        1
      )} Hz, por encima de la frecuencia de Nyquist de ${nyquist.toFixed(
        1
      )} Hz. Las muestras hacen que parezca una onda de ${alias.toFixed(2)} Hz.`;
    } else {
      statusBox.style.borderLeftColor = COLORS.accent3;
      statusBox.innerHTML = `<strong style="color:#7ee787">Fiel.</strong> La señal es de ${state.sigFreq.toFixed(
        1
      )} Hz, por debajo de la frecuencia de Nyquist de ${nyquist.toFixed(1)} Hz — las muestras la reconstruyen correctamente.`;
    }
  }
