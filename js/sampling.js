(function () {
  ...
  function draw() {
    ...
    if (state.sigFreq > nyquist) {
      ...
      statusBox.innerHTML = `<strong style="color:#ff7777">Aliasing.</strong> Signal is ${state.sigFreq.toFixed(
        1
      )} Hz, above the Nyquist frequency of ${nyquist.toFixed(
        1
      )} Hz. The samples make it look like a ${alias.toFixed(2)} Hz wave instead.`;
    } else {
      statusBox.style.borderLeftColor = COLORS.accent3;
      statusBox.innerHTML = `<strong style="color:#7ee787">Faithful.</strong> Signal is ${state.sigFreq.toFixed(
        1
      )} Hz, below the Nyquist frequency of ${nyquist.toFixed(1)} Hz — the samples reconstruct it correctly.`;
    }
  }
